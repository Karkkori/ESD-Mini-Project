...
void setup()
{
    ...
    initHikeWatch();
    SerialBT.begin("Hikkking Watch");

    watch->tft->fillScreen(TFT_BLACK);
    watch->tft->setTextFont(4);
    watch->tft->setTextColor(TFT_WHITE, TFT_BLACK);
    watch->tft->drawString("Hiking watch",  45, 60, 4);
...
void loop()
{
                ...
                if (sessionSent && sessionStored) {
                    ...
                    while(1)
                    {
                        ...
                        if (incomingChar == 'r')
                        {
                            ...
                            watch->tft->setTextFont(4);
                            watch->tft->setTextColor(TFT_WHITE, TFT_BLACK);
                            watch->tft->drawString("Synchronization",  40, 80, 4);
                            watch->tft->drawString("completed", 45, 135);
                            delay(2000);
                            break;
                        }
    ...
    case 4:
    {
        ...
        sessionStored = true;
        watch->tft->fillRect(0, 0, 240, 240, TFT_BLACK);
        watch->tft->drawString("Storing data", 30, 100);
        delay(1000);           
        watch->tft->fillRect(0, 0, 240, 240, TFT_BLACK);
